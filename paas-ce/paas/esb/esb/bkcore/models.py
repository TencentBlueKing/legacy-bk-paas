...
class FunctionController(models.Model):
    ...
    switch_status = models.BooleanField(u'是否开启该功能', default=True,
                                        help_text=u'控制功能是否对外开放，若选择，则该功能将对外开放')
    wlist = models.TextField(u'功能测试白名单', null=True, default='', blank=True,
                             help_text=u'支持两种格式数据，以逗号、分号分隔的字符串，及JSON格式字符串')
...
class ESBBuffetComponent(models.Model):
    ...
    mappings_input = models.CharField(u'输入Mappings', null=True, default='', blank=True,
                                      max_length=1024, help_text=u'JSON格式数据')
    mappings_output = models.CharField(u'输出Mappings', null=True, default='', blank=True,
                                       max_length=1024, help_text=u'JSON格式数据')
    last_modified_time = models.DateTimeField(auto_now=True)
    created_time = models.DateTimeField(auto_now_add=True)
    timeout_time = models.IntegerField(u'超时时长', blank=True, null=True,
                                       help_text=u'单位秒，未设置时以所属组件系统超时时长为准')
    type = models.IntegerField(u'组件类型', choices=TYPE_CHOICE, default=2)
