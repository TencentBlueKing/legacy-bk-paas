...
class ESBBuffetComponentForm(forms.Form):
    ...
    extra_headers = forms.CharField(label=_(u"额外请求头信息"), required=False, widget=forms.HiddenInput())
    extra_params = forms.CharField(label=_(u"额外请求参数"), required=False, widget=forms.HiddenInput())

    registed_path = forms.RegexField(
        label=_(u"注册到的API路径"),
        regex=r"^/[/a-zA-Z0-9_{}-]+$",
        help_text=_(
            u'以斜杠开头，只能包含斜杠、大小写字母、数字、下划线，如："/host/get_host_list/"，可额外包含路径变量，如 "/users/{username}/"；注册到的请求类型+注册到的API路径，在自助接入中需全局唯一'  # noqa
        ),
        required=True,
        max_length=255,
        error_messages={
    ...
    registed_http_method = forms.ChoiceField(label=_(u"注册到的请求类型"), choices=HTTP_METHOD_CHOICES, required=True)

    mappings_input = forms.ModelChoiceField(
        label=_(u"对请求参数应用mapping"), queryset=ESBBuffetMapping.objects.all(), required=False
    )
    mappings_output = forms.ModelChoiceField(
        label=_(u"对返回结果应用mapping"), queryset=ESBBuffetMapping.objects.all(), required=False
    )
    timeout_time = forms.IntegerField(
        label=_(u"超时时长"),
        required=False,
        error_messages={"invalid": _(u"输入格式不正确")},
        min_value=1,
        max_value=86400,
        initial=None,
    ...
    def clean(self):
        ...
        if ESBBuffetComponent.objects.filter(registed_path=value, registed_http_method=method).exists():
...
class EditESBBuffetComponentForm(ESBBuffetComponentForm):
    ...
    def clean(self):
        ...
        if ESBBuffetComponent.objects.exclude(id=id).filter(registed_path=value, registed_http_method=method).exists():
            self._errors["registed_path"] = self.error_class([_(u"路径 %s 已经被占用了，请修改") % value])
            del data["registed_path"]
        return data
...
class ESBBuffetMappingForm(forms.Form):
    ...
    source_type = forms.ChoiceField(
        label=u"源码类型", choices=SOURCE_TYPE_CHOICES, required=True, help_text=(u"目前只支持使用 Jinja2 模板作为 Mappings")
    )
